...
本发布包保留 skill 框架、配置、模板、脚本、冲突登记表、取象规则库和分类占事规则库；不包含用户本地自用语料和运行时记忆。

已剔除或清空：

- `resources/books_raw_md/*.md`：原始 OCR / MinerU 古籍全文。
- `resources/books_corrected/*.md`：人工校正文档。
- `resources/index/*.jsonl|*.json`：本地生成的检索索引。
- `memory/*.jsonl|*.yaml|*.md`：个人案例、反馈、运行时学习记录。
- `raw_pdfs/*`：本地 PDF 原文。
- `notes/*`：个人笔记。

新增模块：

- `knowledge/05_xiangyi_registry/xiangyi_registry.yml`：结构化取象规则。
- `knowledge/06_topic_rules/topic_rules.yml`：分类占事与现代问题映射。
- `resources/templates/fresh_output_template_v2.md`：fresh 模式输出模板。
- `resources/schemas/feedback_schema_v2.json`：反馈记录 schema v2。
- `scripts/validate_pack.py`：发布前结构检查脚本。

本包适合上传 GitHub。若要在本地完整使用，请自行把古籍 OCR、校正文档和索引放回对应目录，并确保 `.gitignore` 不提交这些文件。
